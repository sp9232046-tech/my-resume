...
2\. Фильтр: \`udp.port == 137\` — показываем только NBT-NS трафик (порт 137 UDP)
...
\### Q4: Имя пользователя скомпрометированной учетной записи

\*\*Вопрос:\*\* Какое имя пользователя было у учетной записи, которую взломал злоумышленник?
...
2\. Ищем пакет SMB2 Session Setup Response (№241)
...
2\. Находим пакет Session Setup Response (№241)
...
\- \*\*Фильтрация в Wireshark:\*\*

&#x20; - NBNS-запросы и ответы (порт 137 UDP)

&#x20; - SMB-трафик (порты 445/139 TCP)

&#x20; - Комбинация фильтров для точного поиска
